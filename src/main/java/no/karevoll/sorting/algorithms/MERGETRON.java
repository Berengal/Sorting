...
public class MERGETRON implements SortingAlgorithm {
  ...
  private int threads;
  public MERGETRON(int threads, int parallellCutoff) {
  ...
  public void sort(MemoryArray input, MemoryManager memoryManager) {
    ...
    MemoryArray scratchArray = memoryManager.allocate(input.getSize(),
            "Scratch memory");
    ...
    executor.execute(new Sorter(new MemorySlice(input), new MemorySlice(
            scratchMemory), false, new Counter(1), new Runnable() {
  ...
  private class Sorter implements Runnable {
    ...
    public Sorter(MemorySlice input, MemorySlice scratch, boolean toScratch, Counter counter,
            Runnable afterSort) {
      this.input = input;
    ...
    public void run() {
      ...
      Runnable mergeDone = new Runnable() {
      ...
      final Runnable childrenDone;
      if (toScratch) {
        childrenDone = new Merger(firstSlice, secondSlice, scratch, new Counter(1), mergeDone);
      } else {
        childrenDone = new Merger(firstScratch, secondScratch, input, new Counter(1), mergeDone);
      }
      Counter childCounter = new Counter(2);
      executor.execute(new Sorter(firstSlice, firstScratch, !toScratch, childCounter, childrenDone));
      executor.execute(new Sorter(secondSlice, secondScratch, !toScratch, childCounter, childrenDone));
    }

    private void move(MemorySlice input, MemorySlice scratch) {
      for (int i = 0; i < input.getSize(); i++) {
        scratch.insert(input.remove(i), i);
      }
    }
  ...
  private class Merger implements Runnable {
    ...
    public Merger(MemorySlice firstList, MemorySlice secondList, MemorySlice scratch, Counter counter, Runnable afterMerge) {
    ...
    @Override
    public void run() {
      if (firstList.getSize() < parallellCutoff
              || secondList.getSize() < parallellCutoff) {
        serialMerge();
      } else {
        parallellMerge();
      }
  ...
  private class Mover implements Runnable {
    ...
    public Mover(MemorySlice from, MemorySlice to, Counter counter,
            Runnable doneMoving) {
      this.from = from;
      this.to = to;
      this.counter = counter;
    ...
    private void parallellMove() {
      ...
      executor.execute(new Mover(lowerFrom, lowerTo, childCounter,
              movingDone));
      executor.execute(new Mover(higherFrom, higherTo, childCounter,
              movingDone));
    }
