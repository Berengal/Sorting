...
public class SuperParallellMergeSort implements SortingAlgorithm {
  ...
  public void sort(MemoryArray input, MemoryManager memoryManager) {
    ...
    this.scratchMemory = new MemorySlice(memoryManager.allocate(input.getSize(), "Scratch memory"));
    ...
    executor.execute(new Sorter(new MemorySlice(input), new MemorySlice(scratchMemory), new Counter(1), new Runnable() {
  ...
  private class Sorter implements Runnable {
    ...
    public Sorter(MemorySlice input, MemorySlice scratch, Counter counter, Runnable afterSort) {
      this.input = input;
    ...
    public void run() {
      ...
      Runnable childrenDone = new Merger(firstSlice, secondSlice, scratch, new Counter(1), new Runnable() {
      ...
      executor.execute(new Sorter(firstSlice, firstScratch, childCounter, childrenDone));
      executor.execute(new Sorter(secondSlice, secondScratch, childCounter, childrenDone));
    }
  ...
  class Merger implements Runnable {
    ...
    @Override
    public void run() {
      if (firstList.getSize() + secondList.getSize() < 20) {
        serialMerge();
      } else {
        parallellMerge();
      }
    ...
    private void parallellMerge() {
      ...
      int secondMiddle = binarySearch(secondList, input.read(firstMiddle));
      ...
      int lowerScratchSize = lowerFirstList.getSize() + lowerSecondList.getSize();
      ...
      executor.execute(new Merger(lowerFirstList, lowerSecondList, lowerScratch, mergeCounter, childrenDone));
      executor.execute(new Merger(higherFirstList, higherSecondList, higherScratch, mergeCounter, childrenDone));
    }
    ...
    private int binarySearch(MemorySlice array, Element find) {
      ...
      if (start < array.getSize() - 1 && array.read(start).compareTo(find) == -1) {
        return start + 1;
